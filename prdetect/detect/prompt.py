"""The detector's system prompt: its instructions and the catalogue of defect kinds.

One prompt and one catalogue for every repository. The detector is asked for
every defect it has a real reason to suspect, with an honest confidence, because
every claim is verified afterwards and only what survives is published. Every
instruction and every definition names a defect from the code in front of the
reader; none depends on how the rest of a repository does things, except
duplication, which needs the other copy.

The rendered text is pinned by `tests/test_prompt.py`: a change to it changes
what every run means.
"""
from __future__ import annotations

import hashlib

# How `pack.build` prints a pull request.
WHOLE_FILE_FORMAT = """\
...
def digest() -> str:
    """A short fingerprint of the rendered prompt, recorded in every run's manifest."""
