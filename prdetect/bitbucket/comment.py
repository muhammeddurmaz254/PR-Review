...
conversation -- never as inline comments on the code. A later run updates that
same comment instead of adding another; a comment is recognised as ours by its
first line, `HEADING`.
...
def render(findings: Sequence[dict], head_commit: str, model: str | None, run: str) -> str:
    ...
    if rows:
        ...
        body = ["Bu pull request için bulgu yok."]
...
def upsert(client: CommentClient, workspace: str, repo: str, pull_id: int, raw: str,
           create: bool = True) -> tuple[str, int | None]:
    """Create our comment, or update the one already there. Returns (action, comment id).

    With `create` False an existing comment is still updated -- a table of old
    findings must not stay up -- but no new comment is added.
    """
