"""Print a JSON report from `run_report.py` as plain text.

    python report_txt.py ../reports/genis_olcum_reposu/<run>.json
...
def render(report: dict) -> str:
    ...
    out = [f"Repo      : {report['repository']}",
           f"Koşu      : {report['run']}",
           f"Model     : {report.get('model') or '-'}   prompt {report.get('prompt_version') or '-'}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("report", type=Path, help="a JSON report written by run_report.py")
